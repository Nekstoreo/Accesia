...
public class LoginUserHandler : IRequestHandler<LoginUserCommand, LoginResponse>
{
    ...
    public async Task<LoginResponse> Handle(LoginUserCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            var cooldown =
                await _rateLimitService.GetRemainingCooldownAsync(request.IpAddress, "login", cancellationToken);
        ...
        if (!user.CanLogin())
        {
            ...
            throw new CannotPerformActionException(request.Email, "login",
                "Estado de cuenta no permite iniciar sesión");
        }
        ...
        if (!string.IsNullOrEmpty(request.DeviceName)) session.DeviceName = request.DeviceName;
